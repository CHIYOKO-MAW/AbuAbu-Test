...
@section('title', 'Cover - ' . $tool->title)

@section('content')
<div class="max-w">
    <div class="flex items-center justify-between mb-6">
        <h1>Cover: {{ $tool->title }}</h1>
        <a href="{{ route('admin.tools.show', $tool) }}" class="btn btn-secondary">Back</a>
    </div>

    @php
        $currentCover = $tool->cover_image;
        $hasExistingFile = !empty($currentCover) && !Str::startsWith($currentCover, ['http://', 'https://']);
        $isUrl = Str::startsWith($currentCover, ['http://', 'https://']);
    @endphp

    <div class="card p-4">
        <form method="POST" action="{{ route('admin.tools.uploadCover', $tool) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="w-48 h-48 bg-surface-hover rounded-lg overflow-hidden flex items-center justify-center">
                    @if($currentCover)
                        @if($isUrl)
                            <img src="{{ $currentCover }}" alt="{{ $tool->title }}" class="w-full h-full object-cover">
                        @else
                            <img src="{{ asset('storage/' . $currentCover) }}" alt="{{ $tool->title }}" class="w-full h-full object-cover">
                        @endif
                    @else
            ...
            <div class="flex justify-between">
                <a href="{{ route('admin.tools.show', $tool) }}" class="btn btn-secondary">Cancel</a>
                <button type="submit" class="btn btn-primary">Save Cover</button>
            </div>
        </form>

        @if($currentCover)
        <form method="POST" action="{{ route('admin.tools.deleteCover', $tool) }}" class="mt-4 pt-4 border-t border-border">
            @csrf
            @method('DELETE')
