...
@section('title', 'Cover - ' . $item->title)

@section('content')
<div class="max-w">
    <div class="flex items-center justify-between mb-6">
        <h1>Cover: {{ $item->title }}</h1>
        <a href="{{ route('admin.reading.show', $item) }}" class="btn btn-secondary">Back</a>
    </div>
    ...
    <div class="card p-4">
        <form method="POST" action="{{ route('admin.reading.uploadCover', $item) }}" enctype="multipart/form-data">
            @csrf

            <div class="mb-6">
                <label class="block text-sm mb-2">Current Cover</label>
                <div class="w-48 h-64 bg-surface-hover rounded-lg overflow-hidden flex items-center justify-center">
                    @if($currentCover)
                        @if($isUrl)
                            <img src="{{ $currentCover }}" alt="Cover" class="w-full h-full object-cover">
                        @else
                            <img src="{{ asset('storage/' . $currentCover) }}" alt="Cover" class="w-full h-full object-cover">
                        @endif
                    @else
            ...
            <div class="flex justify-between">
                <a href="{{ route('admin.reading.show', $item) }}" class="btn btn-secondary">Cancel</a>
                <button type="submit" class="btn btn-primary">Save Cover</button>
            </div>
        </form>

        @if($currentCover)
        <form method="POST" action="{{ route('admin.reading.deleteCover', $item) }}" class="mt-4 pt-4 border-t border-border">
            @csrf
            @method('DELETE')
